...
add(P("On the margin: it is " + f(C.margin,2) + " on the LOG hazard ratio, roughly a five per cent relative change, chosen as a difference below which a protocol decision would not change. The scale is now stated wherever the margin appears, which it was not before. The margin is applied only to the observed-cohort frontier statistics; the simulation figures use a bare inequality, and the text now says so rather than leaving the reader to infer it."));
if (TW) {
