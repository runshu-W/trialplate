...
/* 1 */
add(Q("1. The internally validated three-way results have no patient-level uncertainty, yet the Methods say every out-of-sample quantity passes through the outer bootstrap."));
...
add(P("One thing the run makes visible that we would rather state than have found: these rates are poorly determined. The bootstrap uses " + (TN ? TN.colon.R : "25") + " inner splits per resample against the " + (TW ? f0(TW.colon.R) : "400") + " of the point-estimate run, and its own observed-data replicate differs from that estimate by up to " + (NUM.threeway_nested && NUM.threeway_nested.max_gap ? f(NUM.threeway_nested.max_gap,2) : "\u2014") + " across the three internally validated quantities. The intervals say the same thing more formally. We report the gap in the caption rather than leaving a reader to notice that two runs of the same quantity disagree, which is the failure mode of the last two rounds."));
...
add(P("The title now reads “…and why reliability was not determined by cohort size alone across the mechanisms examined”. The abstract says that no universal requirement could be determined from cohort size, event count or effective sample size alone across the mechanisms simulated. The Results and the Conclusions say that what the simulations rule out is a portable cohort-size-only threshold of the kind we ourselves offered in an earlier version, within the mechanisms examined, and add explicitly that this is a statement about those mechanisms and not a general impossibility result."));
