...
const CUR = (NUM.fixedn||{}).currency_detail || null, CDIF = (NUM.fixedn||{}).currency_diff || null;
...
if (CUR) {
  const blk = (key, lab) => {
    const D = CUR[key];
    add(P(lab + " \u2014 bin edges " + D.edges.map(e => f0(e)).join(", "), {b:true, after:70, size:19}));
...
if (CDIF) add(P("The three summaries are " + f(NUM.fixedn.spread_patients,3) + ", " + f(NUM.fixedn.spread_events,3) + " and " + f(NUM.fixedn.spread_ess,3) + ", so the observed differences are " + f(CDIF.events_minus_patients,3) + " for events minus patients and " + f(CDIF.ess_minus_patients,3) + " for effective sample size minus patients. A cluster bootstrap over the " + f0(CDIF.n_clusters) + " scenarios, which is the resampling unit because each contributes four rows, gives intervals [" + f(CDIF.ci_events[0],3) + ", " + f(CDIF.ci_events[1],3) + "] and [" + f(CDIF.ci_ess[0],3) + ", " + f(CDIF.ci_ess[1],3) + "]. Both contain zero and are wide. Sixteen scenarios cannot establish that the three currencies are equivalent, and we no longer say they are: what the comparison supports is that none of the three separated the scenarios better than the others in this design.", {after:130}));
if (CDIF && CDIF.boot_mean_events !== undefined) add(P("One detail of the bootstrap is worth stating because it looks like an error otherwise. The quartile edges are recomputed inside each replicate, and a replicate that draws some scenarios twice and others not at all cuts the bins differently, which moves the within-bin spread. The bootstrap means are therefore not the observed differences and here fall on the other side of zero: " + f(CDIF.boot_mean_events,3) + " and " + f(CDIF.boot_mean_ess,3) + " against " + f(CDIF.events_minus_patients,3) + " and " + f(CDIF.ess_minus_patients,3) + ". The point estimates above are the observed differences; the bootstrap is used only for the intervals. The gap between the two is itself much smaller than the interval width, which is the reason the comparison settles nothing either way.", {after:170}));

add(H1("Table S3. Matched and frontier comparison, with approximate patient-level uncertainty"));
...
if (NS.colon && PR.colon) { const C=NS.colon, RT=NS.rott, PC=PR.colon, PRT=PR.rott;
  ...
  const ivs = z => "[" + f(z.ci[0],2) + ", " + f(z.ci[1],2) + "]" +
    (z.ci_deflated ? " / [" + f(z.ci_deflated[0],2) + ", " + f(z.ci_deflated[1],2) + "]" : " / \u2014");
...
add(H1("Table S3c. Three-way split: three frontier quantities, kept apart, with approximate patient-level uncertainty"));
...
if (NUM.threeway) { const A = NUM.threeway.colon, B = NUM.threeway.rott;
  ...
  const ivs2 = z => "[" + f(z.ci[0],2) + ", " + f(z.ci[1],2) + "]" +
    (z.ci_deflated ? " / [" + f(z.ci_deflated[0],2) + ", " + f(z.ci_deflated[1],2) + "]" : " / \u2014");
  ...
  add(table(["quantity","colon","Rotterdam (descriptive)"],
   ...
   [5000, 2200, 2160],
   "Only block (b) evaluates the rule on patients that took no part in choosing its comparator. It is an internally validated sample-split estimate, not a confirmatory result in an independent cohort, since the same patients are reused across splits. Block (a) is the empirical oracle whose optimism is being measured; block (c) is a second empirical oracle on a different third, reported so a reader can see that the counts are a property of any part of these cohorts rather than evidence about the rule. Point estimates come from the repeated three-way splits in the first row; intervals in block (b) come from an outer bootstrap over patients wrapped around the whole fit / select / test procedure, and are given raw then deflated, as in Table S3 and for the same reason — including the two dashes, which here are of the second kind described in that note: the lowest-hazard-ratio dominator moves so much between inner splits that the inner component alone accounts for the whole spread across resamples, and its raw interval should be read as reflecting that noise rather than patient-level sampling — each outer replicate uses " + (TN ? f0(TN.colon.R) : "25") + " inner splits against the " + f0(A.R) + " behind the point estimate, so the raw interval carries inner Monte Carlo variance that a population interval should not. As elsewhere, the bootstrap supplies uncertainty only and never a point estimate, and the export asserts that its own observed-data replicate agrees with the repeated-split estimate to within its Monte Carlo noise. That agreement is not close, and the reader should know it: the bootstrap runs " + (TN ? TN.colon.R : "25") + " inner splits per resample against the " + f0(A.R) + " of the point-estimate run, and the two differ by up to " + (NUM.threeway_nested && NUM.threeway_nested.max_gap ? f(NUM.threeway_nested.max_gap,2) : "\u2014") + " across these quantities. These rates are poorly determined at a third of a cohort, and the intervals show it. The endpoints themselves are also poorly determined: the row above gives the Monte Carlo standard error of the 2.5% and 97.5% endpoints, obtained by resampling the " + (TN ? TN.colon.B : 100) + " outer replicates, and at that outer count it is large enough that the endpoints should be read to one decimal at most."));
}
