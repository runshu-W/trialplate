...
const CV = NUM.converge || null, USE = NUM.usable || null;
...
function table(head, rows, widths, note, sz) {
  /* A cell value may be an array, which stacks as separate lines inside the cell.
     Reviewer round 9 asked for the observed and the variance-adjusted ranges to be
     kept in separate columns rather than joined by a slash, and the tails then need
     a line of their own. */
  const cell = (txt, o={}) => new TableCell({width:{size:o.w,type:WidthType.DXA},
    ...
    children:(Array.isArray(txt)?txt:[txt]).map((line,j)=>new Paragraph({
      spacing:{after:0,line:240},
      alignment:o.right?AlignmentType.RIGHT:AlignmentType.LEFT,
      children:[new TextRun({text:String(line), bold:o.head,
        size:(j?(sz??16)-2:(sz??16)), color:j?"6B7680":undefined, font:"Calibri"})]}))});
...
if (CV) {
  ...
  add(table(["quantity","per-split SD"].concat(shared.map(c => "R = " + f0(c))).concat(["full run"]),
    ...
    "Colon cohort. Each ladder cell gives the implied Monte Carlo standard deviation s/√R, then after the slash the observed standard deviation of the independent block estimates with the number of blocks in parentheses. The last column is each quantity's own full run, which does not divide into two or more blocks and so carries no observed spread; a dash after the slash in this table means only that, and is unrelated to the dash in Tables S3 and S3c, which marks a non-identifiable variance component. The observed spread is itself imprecise where the blocks are few: a standard deviation from two to five blocks carries one to four degrees of freedom and can sit well above or below the implied value without meaning anything. It is the columns with many blocks that carry the comparison. Writing the ratio of the observed spread to the implied one, it runs " + (function(){ const rr = CV.rows.filter(z => z.blk_n >= 10 && z.mc_sd > 0 && isFinite(z.blk_sd)).map(z => z.blk_sd / z.mc_sd); return f(Math.min.apply(null,rr),2) + " to " + f(Math.max.apply(null,rr),2); })() + " over the cells resting on ten blocks or more, and " + (function(){ const rr = CV.rows.filter(z => z.blk_n >= 2 && z.mc_sd > 0 && isFinite(z.blk_sd)).map(z => z.blk_sd / z.mc_sd); return f(Math.min.apply(null,rr),2) + " to " + f(Math.max.apply(null,rr),2); })() + " over every cell in the table. The extremes at both ends sit in the columns with fewest blocks, where the observed spread carries one to four degrees of freedom and means little. Where the implied value is the larger, the correction removes more variance than the blocks show. We do not translate that into a direction for coverage, because as the last paragraph of this section says, a statement about second moments does not determine the coverage of a shrunken quantile; it is a reason the observed patient-resampled range is always printed beside the variance-adjusted sensitivity range in Tables S3 and S3c. A few splits are unusable, so a column can rest on slightly fewer than its nominal count; the count actually behind each cell is in the result file."));
...
/* ---------------- Table S1g: usable inner splits ----------------
   Reviewer round 10, point 3. An inner split can fail to define a statistic, and
   the counts were only in the result file. They belong in the paper, because they
   are the denominator of the Monte Carlo term. */
add(H1("Table S1g. Inner splits that define each statistic"));
add(P("Inside an outer resample the procedure is run over a fixed number of inner splits, but a split does not always define every statistic. A matched comparison is undefined in a split where no other subset falls inside the matching band; a replication rate is undefined where no subset dominated the rule on the selection third, so there is nothing to reproduce. Two consequences follow and both are worth stating rather than leaving in the result file. A resample's value for a statistic is the mean over the splits that DO define it, not over all of them. And the inner Monte Carlo term for that resample is its own sample variance divided by its own usable count, s_b squared over R_b, never over the nominal count; the same holds for the deflation that Tables S3 and S3c report."));
/* Reviewer round 10 audit: the table published raw result-matrix column names and
   gave no sense of how OFTEN a statistic falls short. Both fixed here. */
const QNAME = {"rank_hr_0.02":"matched on HR, band 2%", "rank_rm_0.02":"matched on RMST, band 2%",
               "skew_0.02":"comparator count skew, band 2%",
               "repro":"replication rate", "reproM":"replication rate with the margin required",
               "best_holds":"lowest-hazard-ratio dominator carries over"};
const qn = k => QNAME[k] || k;
if (USE) {
  const rows = Object.keys(USE).map(k => { const z = USE[k];
    return [z.design + ", " + (z.cohort === "rott" ? "Rotterdam" : "colon"),
            f0(z.n_resamples) + " × " + f0(z.R_nominal),
            f0(z.n_full_cols) + " of " + f0(z.n_cols),
            z.all_full ? "—"
              : z.short.map(q => qn(q.quantity) + ": " + f0(q.min) + " to " + f0(q.max)
                  + ", median " + f0(q.median) + ", short in " + f0(q.n_below) + " of " + f0(z.n_resamples)).join("; ")]; });
  add(table(["design and cohort","bootstrap resamples × nominal inner splits","statistics at the nominal count in every resample","statistics that ever fall short: usable count, median, and how often"],
    rows, [1900, 1500, 1600, 4360],
    "Counts are over the bootstrap resamples only; the observed-data evaluation is excluded, as it is everywhere else. The three-way quantities do not share a denominator and should not be read as though they did. The replication rate and the lowest-hazard-ratio carry-over are defined only in splits where at least one subset dominated the rule on the selection third. The replication rate with the margin required is defined only where at least one dominator also clears the margin, a strictly smaller set of splits, which is why its usable counts are the lowest in the table. Frontier membership on the pre-selected subsets is defined in every split: a split producing no dominator leaves the rule undominated by construction and is recorded as such, not dropped. “Short in k of n” counts RESAMPLES in which at least one inner split failed to define the statistic, not inner splits: the median usable count in the same cell shows how many of the nominal " + f0(USE.three_colon.R_nominal) + " a typical resample kept, which for every quantity in this table is " + f0(Math.min.apply(null, Object.keys(USE).reduce(function(a2,k){return a2.concat(USE[k].short.map(function(q){return q.median;}));}, []))) + " or more.  The denominators “of " + f0(USE.two_colon.n_cols) + "” and “of " + f0(USE.three_colon.n_cols) + "” count every column of the result matrix, including the comparator and dominator counts that are reported as means rather than as probabilities. "
    + "No statistic is undefined in a whole resample in any design or cohort — the export asserts this over all four result objects and stops the build if it ever ceases to hold — so every resample contributes to every range in Tables S3 and S3c. "
    + "The statistics that fall short are the ones whose definitions can fail: the tightest matching band, where a split may offer no comparator within two per cent of the rule’s eligible count, and the three quantities that require a dominator on the selection third, which are undefined together when a split produces none. "
    + "Frequency matters as much as depth here. In the two-way design the shortfall is rare. In the three-way design it is not: " + qn(USE.three_colon.worst) + " is below the nominal count in " + f0((USE.three_colon.short.filter(q => q.quantity === USE.three_colon.worst)[0]||{}).n_below) + " of " + f0(USE.three_colon.n_resamples) + " colon resamples and " + f0((USE.three_rott.short.filter(q => q.quantity === USE.three_rott.worst)[0]||{}).n_below) + " of " + f0(USE.three_rott.n_resamples) + " Rotterdam resamples, being the most demanding definition of the three. "
    + "Table S1e sizes the inner Monte Carlo component at the nominal count throughout; where the usable count is smaller the true component is a little larger, which is a further reason to read the deflation as approximate."));
}
...
if (CUR) {
  const blk = (key, lab) => {
    const D = CUR[key];
    add(P(lab + " \u2014 bin edges (three significant figures; the bin labels in the table use R's own formatting) " + D.edges.map(e => { const v = Number(e); const t = Number(v.toPrecision(3)); return String(t); }).join(", "), {b:true, after:70, size:19}));
...
if (LOSO) add(P("What the fixed design does support is a sensitivity analysis. Recomputing the three summaries with each scenario left out in turn, the events-minus-patients difference moves over " + f(LOSO.range_events[0],3) + " to " + f(LOSO.range_events[1],3) + " and the effective-sample-size difference over " + f(LOSO.range_ess[0],3) + " to " + f(LOSO.range_ess[1],3) + ", against observed values of " + f(LOSO.events_minus_patients,3) + " and " + f(LOSO.ess_minus_patients,3) + ". No single scenario is carrying the similarity. One thing the sweep does show, and we state it rather than leave it in the table: the observed events-minus-patients difference is small enough that omitting any one of " + (function(){ const r = LOSO.rows.filter(z => z.d_events < 0).map(z => f0(z.run)); return r.length + " scenarios (" + r.join(", ") + ")"; })() + " reverses its sign, the largest reversal being to " + f(LOSO.range_events[0],3) + " against an observed " + f(LOSO.events_minus_patients,3) + ". The magnitude stays small throughout, but the sign of a difference this size carries no information and should not be read as one. That is the whole of what we claim: THE THREE CHOSEN DESCRIPTIVE SUMMARIES WERE NUMERICALLY SIMILAR UNDER THIS PARTICULAR BINNING SCHEME, and were not driven by any single scenario. It is not a claim that the three currencies are equivalent, nor that none of them separates scenarios better than the others; the binning is not even the same kind of quantity across the three, since the patients column is a within-fitting-size dispersion and the other two are not. If uncertainty is wanted here, the honest route is to propagate each scenario's own Monte Carlo error under the fixed design rather than to resample the design.", {after:170}));
...
if (NS.colon && PR.colon) { const C=NS.colon, RT=NS.rott, PC=PR.colon, PRT=PR.rott;
  ...
  /* Reviewer round 9. The observed range and the variance-adjusted one now get
     columns of their own rather than a slash, and are named differently, because
     the adjusted quantity is not an empirical quantile of the resampled statistic:
     it is a heuristic transformation whose quantile accuracy is unknown. The
     observed range is the one the main text quotes. */
  const obs = z => ["[" + f(z.ci80[0],2) + " to " + f(z.ci80[1],2) + "]",
                    "tails " + f(z.ci[0],1) + "\u2013" + f(z.ci[1],1)];
  const adj = z => z.ci80_deflated
    ? "[" + f(z.ci80_deflated[0],2) + " to " + f(z.ci80_deflated[1],2) + "]" : "\u2014";
  const row = (lab, key) => [lab,
    [f(pt(PC,C,key),3)].concat(obs(C[key])), adj(C[key]),
    [f(pt(PRT,RT,key),3)].concat(obs(RT[key])), adj(RT[key])];
  const plain = (lab, a, b2) => [lab, a, "", b2, ""];
  add(table(["quantity",
             "colon: point + observed patient-resampled range",
             "colon: variance-adjusted sensitivity range (heuristic)",
             "Rotterdam (descriptive): point + observed range",
             "Rotterdam: variance-adjusted (heuristic)"],
   [plain("splits behind the point estimate", f0(PC.R), f0(PRT.R)),
    plain("bootstrap resamples x inner splits, forming the ranges", f0(C.lower.n_outer)+" x "+C.R, f0(RT.lower.n_outer)+" x "+RT.R),
    plain("plus one observed-data evaluation, held out of the ranges", "1", "1"),
    ...
    plain("matched comparators per split (mean, 10% band)", f0(PC["nm0.1"]), f0(PRT["nm0.1"])),
    plain("comparator count skew vs rule", f(PC["sk0.1"],3), f(PRT["sk0.1"],3)),
    plain("subsets dominating (mean)", f0(PC.n_dom), f0(PRT.n_dom))].concat(
      C.d_pair_greater ? [plain("PAIRED difference, RMST rule minus published rule", "", ""),
      ...
      C.rm_same ? [row("RMST rule selects the same set","rm_same"),
                   plain("RMST rule: eligible patients (mean)", f0(PC.n_rmrule), f0(PRT.n_rmrule)),
                   row("RMST rule: P(lower hazard ratio)","rmrule_lower"),
                   row("RMST rule: P(greater RMST)","rmrule_greater")] : []).concat(
      isFinite(C.lower.conv) ? [plain("largest endpoint movement, half vs full outer count",
        f(maxconv(C),3), f(maxconv(RT),3))] : []),
   [2860, 1900, 1350, 1900, 1350],
   "Each row gives the point estimate with the OBSERVED PATIENT-RESAMPLED RANGE below it \u2014 the central 80% of the outer bootstrap distribution, then the 2.5 to 97.5 per cent span to one decimal \u2014 and, in its own column, the VARIANCE-ADJUSTED SENSITIVITY RANGE. The two are named differently and kept apart because they are different kinds of object. The observed range is an empirical quantile of the resampled statistic. The adjusted range is not: it is the same replicates shrunk toward their mean by the ratio of the outer-sampling to the total standard deviation, a heuristic that matches a second moment and whose quantile accuracy and coverage are unknown. It may be either too wide or too narrow, and the main text quotes the observed range throughout, never this one. Neither is a confidence interval. They describe how far each statistic moves when the cohort is resampled over patient identifiers and the whole split procedure is rerun; we make no coverage claim for either and nothing in the paper turns on where an endpoint falls. Two further reasons the observed range is not a population range: each outer replicate is a " + f0(C.R) + "-split estimator at nominal count, sometimes fewer (Table S1g), rather than the " + f0(PC.R) + "- and " + f0(PRT.R) + "-split estimators behind the point estimates, so the spread carries inner Monte Carlo variance that patient sampling alone would not (Table S1e quantifies that component); and coverage has not been evaluated against a known population at all. A dash in the adjusted column means the estimated inner Monte Carlo variance was at least the total spread, so the subtraction returns a non-positive outer-sampling variance and the patient-level component is NOT IDENTIFIABLE at this Monte Carlo precision. That is neither a finding that patient sampling contributes nothing nor a demonstration that the inner term accounts for the whole spread, both of which we have said in earlier versions and withdraw. The central 80% leads because at " + f0(C.lower.n_outer) + " bootstrap resamples its endpoints rest on the fifteenth replicate from each end while the 2.5 and 97.5 points rest on the fourth; the Monte Carlo error of the wider endpoints, from resampling the outer replicates, is at most " + f(Math.ceil(100*Math.max.apply(null, Object.keys(C).filter(k => C[k] && C[k].ci_se).map(k => Math.max(C[k].ci_se[0], C[k].ci_se[1]))))/100,2) + " over every quantity in the colon result object, against " + f(Math.ceil(100*Math.max.apply(null, Object.keys(C).filter(k => C[k] && C[k].ci80_se).map(k => Math.max(C[k].ci80_se[0], C[k].ci80_se[1]))))/100,2) + " for the central ones, which is why the wider span is printed to one decimal. The second decimal of the central endpoints is inside their Monte Carlo error for the least well determined quantities too; we print it so that narrow ranges stay distinguishable from one another, not because that digit is determined. The " + f0(C.B) + " outer replicates comprise the observed cohort itself as the first, which is held out of the range and used only for the agreement check described under Table S3c, and " + f0(C.lower.n_outer) + " bootstrap resamples that form the range. No outer replicate failed or was discarded; a previous version of this supplement said one was dropped in each design where the split procedure failed, and that was wrong. Point estimates are from the primary analysis; the outer bootstrap supplies spread only. A range is centred on the bootstrap\u2019s own mean, which comes from a noisier estimator than the point estimate beside it, so for a quantity near a boundary the adjusted range can sit wholly to one side of that point; the count/RMST frontier row in colon and the count/HR frontier row in Rotterdam both do. A quantity that takes the same value in every replicate, such as the Rotterdam probability of more eligible patients, has a central range of zero width. SD is decomposed as SD_outer = sqrt(SD_total^2 - SD_MC^2), with SD_MC^2 estimated from the sample variance of the per-split values within each resample rather than from p(1-p)/R. The last row reports how far either of the wider endpoints moves between half and the full outer count.")); }
...
if (NUM.threeway) { const A = NUM.threeway.colon, B = NUM.threeway.rott;
  ...
  /* Reviewer round 9: observed range and variance-adjusted range in separate
     columns, named differently, as in Table S3. */
  const obs2 = z => ["[" + f(z.ci80[0],2) + " to " + f(z.ci80[1],2) + "]",
                     "tails " + f(z.ci[0],1) + "\u2013" + f(z.ci[1],1)];
  const adj2 = z => z.ci80_deflated
    ? "[" + f(z.ci80_deflated[0],2) + " to " + f(z.ci80_deflated[1],2) + "]" : "\u2014";
  const tn4 = (key, a, b) => (TN && TN.colon[key] && TN.rott[key])
    ? [[f(a,3)].concat(obs2(TN.colon[key])), adj2(TN.colon[key]),
       [f(b,3)].concat(obs2(TN.rott[key])), adj2(TN.rott[key])]
    : [f(a,3), "", f(b,3), ""];
  const pl4 = (lab, a, b2) => [lab, a, "", b2, ""];
  add(table(["quantity",
             "colon: point + observed patient-resampled range",
             "colon: variance-adjusted (heuristic)",
             "Rotterdam (descriptive): point + observed range",
             "Rotterdam: variance-adjusted (heuristic)"],
   [pl4("three-way splits (point estimate)", f0(A.R), f0(B.R)),
    pl4("bootstrap resamples x inner splits, forming the ranges",
     TN ? f0(TN.colon.front_conf.n_outer) + " x " + TN.colon.R : "\u2014", TN ? f0(TN.rott.front_conf.n_outer) + " x " + TN.rott.R : "\u2014"),
    pl4("plus one observed-data evaluation, held out of the ranges", "1", "1"),
    pl4("(a) selection third \u2014 empirical, optimistic", "", ""),
    pl4("      dominating subsets (median)", f0(A.dom_sel), f0(B.dom_sel)),
    ["      rule undominated"].concat(tn4("front_sel", A.front_sel, B.front_sel)),
    pl4("(b) pre-selected subsets on the untouched third \u2014 internally validated", "", ""),
    pl4("      of those dominators, number still dominating (median)", f0(A.n_survive), f0(B.n_survive)),
    ["      replication rate"].concat(tn4("repro", A.repro, B.repro)),
    ["      replication rate, margin required on the selection third"].concat(tn4("reproM", A.reproM, B.reproM)),
    ["      lowest-hazard-ratio dominator carries over"].concat(tn4("best_holds", A.best_holds, B.best_holds)),
    ["      rule undominated by every pre-selected subset"].concat(tn4("front_conf", A.front_conf, B.front_conf)),
    pl4("      largest endpoint movement, half vs full outer count",
    ...
    pl4("      largest Monte Carlo error of a wider endpoint",
    ...
    pl4("(c) fresh scan of all 512 on the test third \u2014 descriptive only", "", ""),
    pl4("      dominating subsets (median)", f0(A.dom_test), f0(B.dom_test)),
    ["      rule undominated"].concat(tn4("front_test", A.front_test, B.front_test))],
   [2860, 1900, 1350, 1900, 1350],
   "The three quantities in block (b) are conditioned differently and the difference matters. The replication rate, the margin-constrained replication rate and the lowest-hazard-ratio carry-over are all conditional on the selection third producing a dominator at all \u2014 the margin-constrained one on its producing a dominator that also clears the margin \u2014 whereas frontier membership is defined in every split, a split with no dominator being recorded as one in which the rule is undominated. Table S1g gives the resulting split counts. Only block (b) evaluates the rule on patients that took no part in choosing its comparator. It is an internally validated sample-split estimate, not a confirmatory result in an independent cohort, since the same patients are reused across splits. Block (a) is the empirical oracle whose optimism is being measured; block (c) is a second empirical oracle on a different third, reported so a reader can see that the counts are a property of any part of these cohorts rather than evidence about the rule. Point estimates come from the repeated three-way splits in the first row. The ranges in block (b) come from an outer bootstrap over patients wrapped around the whole fit / select / test procedure and are read exactly as in Table S3: the observed patient-resampled range \u2014 central 80%, with the 2.5 to 97.5 per cent span beneath it to one decimal \u2014 and, in its own column, the variance-adjusted sensitivity range, which is a heuristic transformation of the same replicates rather than an empirical quantile of them. Neither is a confidence interval, and the text quotes the observed range. The two dashes are on the lowest-hazard-ratio dominator, whose estimated inner Monte Carlo variance is at least its total spread across resamples. That means its patient-level variance component is not identifiable at this Monte Carlo precision. An earlier version of this note read it as the inner term accounting for the whole spread; that was wrong, and no conclusion should be drawn from it in either direction. Each outer replicate uses a nominal " + (TN ? f0(TN.colon.R) : "25") + " inner splits, and fewer where a split defines no dominator (Table S1g), against the " + f0(A.R) + " and " + f0(B.R) + " behind the point estimates, so the observed spread carries inner Monte Carlo variance that patient sampling alone would not. The " + (TN ? f0(TN.colon.B) : 100) + " outer replicates comprise the observed cohort itself as the first, used only for the agreement check below, and " + (TN ? f0(TN.colon.front_conf.n_outer) : "99") + " bootstrap resamples that form the ranges. No outer replicate failed or was discarded; a previous version of this supplement said one was dropped for failure, and that was wrong. As elsewhere the bootstrap supplies spread only and never a point estimate. The export checks that the bootstrap\u2019s own observed-data replicate agrees with the repeated-split estimate to within the Monte Carlo noise of a " + (TN ? f0(TN.colon.R) : "25") + "-split estimator, and it passes; but that tolerance is wide, and the gap it tolerates is large in absolute terms \u2014 the two differ by up to " + (NUM.threeway_nested && NUM.threeway_nested.max_gap ? f(NUM.threeway_nested.max_gap,2) : "\u2014") + " across these quantities. We report the size rather than leaving a reader to infer from a passing check that the two agree closely, because they do not. These rates are poorly determined at a third of a cohort. So are the wider endpoints: the row above gives their Monte Carlo standard error, obtained by resampling the " + (TN ? f0(TN.colon.front_conf.n_outer) : 99) + " bootstrap replicates, which is why the central 80% range leads and the wider span is printed to one decimal."));
}
...
add(P("Seven checks run whenever the numbers are exported and stop the build if violated. Three are on the exported numbers: the Rotterdam endpoint must be the death time paired with the death indicator; the horizons must be 1825 and 2555 days; and the primary and nested estimates of the same quantity must agree to within a fixed tolerance, which for the two-way quantities is 0.25 on a proportion \u2014 a loose bound rather than a Monte Carlo calculation, and the observed gaps are reported in Table S3c rather than left to it. The fourth is on the source, because a check on the numbers alone would not have caught the error that prompted it: every analysis script is scanned and the export refuses if any line pairs the relapse-free time with the death indicator, or calls a Rotterdam dataset at the colon horizon. The fifth refuses to run if any of the five superseded result objects reappears in the read path. The sixth asserts " + (NUM.integers ? f0(Object.keys(NUM.integers).length) : "the") + " integer counts — both cohort sizes, the criterion and subset counts, both horizons, every split and resample count including those of the three-way design, the simulation replicate counts and the number of factorial scenarios — against the analysis output, because the provenance check over the manuscript sources tests decimals and percentages but not bare integers. The seventh asserts that no statistic is undefined in a whole outer resample, so every resample contributes to every range reported in Tables S3 and S3c; the usable counts behind it are Table S1g. Separately from the export, analysis/check_outer.R reads the four stored result objects and prints the claims made about them here \u2014 that no replicate failed or was discarded, that the first row of each is the observed-data evaluation rather than a resample, and the usable inner-split counts \u2014 so a reader can confirm them without rerunning any analysis.", {after:120}));
