...
const _NSC = (NUM.nested||{}).colon || {}, _NSR = (NUM.nested||{}).rott || {};
const _w = z => (z && z.ci80) ? (z.ci80[1] - z.ci80[0]) : NaN;
const _PAIR = [_NSC.d_pair_lower, _NSC.d_pair_greater, _NSR.d_pair_lower, _NSR.d_pair_greater];
const _MARG = [_NSC.lower, _NSC.greater, _NSR.lower, _NSR.greater,
               _NSC.rmrule_lower, _NSC.rmrule_greater, _NSR.rmrule_lower, _NSR.rmrule_greater];
...
"We implemented the published rule — retain every criterion whose Shapley value on the log hazard ratio is negative — and evaluated it by repeated stratified sample splitting in two fully public cohorts: a randomised adjuvant colon-cancer trial (n = 619) and the Rotterdam breast-cancer registry (n = 2982). Because the eligible count is monotone in the criteria removed, the rule was compared not against the full protocol alone but against all 512 subsets scored on the held-out half, matched on eligible count and placed against the empirical held-out frontier, whose optimism is quantified separately in simulation. Every out-of-sample quantity passes through an outer bootstrap split by patient identifier, whose spread is reported as a range and not as an interval: each range carries the Monte Carlo variance of the inner splits, and its coverage has not been established. Simulation with a fixed 100 000-patient scoring set isolated fitting-cohort size in one generating mechanism, and a resolution-IV factorial in six factors varied that mechanism, reported as success probability at fixed sizes, and a further sweep varied how a fixed total of effect modification is arranged.")]));
add(P([R("Results. ", {b:true}), R(
"Matched on eligible count and carried through resampling of the patients, the rule reached a lower hazard ratio than a comparable subset in " + _pc(_PRC["hr0.1"]) + " of colon comparisons and " + _pc(_PRR["hr0.1"]) + " in Rotterdam, averaging within-split proportions with equal weight; on the restricted mean difference the two cohorts point opposite ways, " + _pc(_PRC["rm0.1"]) + " and " + _pc(_PRR["rm0.1"]) + ". Resampling the patients and rerunning the split procedure moves each of these figures over a central range " + _f2(Math.min.apply(null,[_NSC["hr0.1"],_NSR["hr0.1"],_NSC["rm0.1"],_NSR["rm0.1"]].map(_w))) + " to " + _f2(Math.max.apply(null,[_NSC["hr0.1"],_NSR["hr0.1"],_NSC["rm0.1"],_NSR["rm0.1"]].map(_w))) + " wide on a scale running from 0 to 1, part of that width being the Monte Carlo noise of the inner splits rather than the patient sample; we report the movement as a range rather than as a confidence interval, drawing no conclusion about any population value from where its endpoints fall. Its choice was almost never on the empirical held-out frontier, but that frontier is optimistic: in simulation only " + _pc(_FO.true_frac) + " of apparently dominating subsets genuinely dominate at the population and " + _pc(_FO.repro) + " reproduce on an independent half, and in a three-way split of the cohorts themselves, where the comparator subsets are fixed on separate data before evaluation, the rule is left undominated in " + (_TW ? _pc(_TW.colon.front_conf) + " of colon and " + _pc(_TW.rott.front_conf) + " of Rotterdam splits, against " + _pc(_TW.colon.front_sel) + " and " + _pc(_TW.rott.front_sel) + " judged on the selection third of that same design" : "a substantially larger share of splits") + ". A variant selecting on the restricted mean difference agreed with the published rule on the criterion set in " + _pc(_PRC.same_set) + " of colon splits, and its point estimates ran in opposite directions on the two estimands: the probability of improvement was higher for it on the restricted mean difference and lower on the hazard ratio, the paired difference between the two rules, carried through the same outer bootstrap, moving over central ranges " + _f2(Math.min.apply(null,_PAIR.map(_w))) + " to " + _f2(Math.max.apply(null,_PAIR.map(_w))) + " wide against " + _f2(Math.min.apply(null,_MARG.map(_w))) + " to " + _f2(Math.max.apply(null,_MARG.map(_w))) + " for the eight marginal figures the two rules contribute. No equivalence margin was prespecified and no equivalence test was run, so neither an advantage nor equivalence is claimed. The observed proportion of splits in which the data-driven protocol reached a lower out-of-sample hazard ratio than the original was below one half in colon (" + _PRC.lower.toFixed(3) + ") and close to one half in Rotterdam (" + _PRR.lower.toFixed(3) + "); under the same resampling-and-splitting procedure both move over more than half the unit interval, so these cohorts do not pin the effect-estimate promise down in either direction. Across the factorial the success probability varied by between " + _f2((NUM.fixedn||{}).sd_min) + " and " + _f2((NUM.fixedn||{}).sd_max) + " in standard deviation across the fixed sample sizes, and the descriptive dispersion measure we used was numerically similar for the three currencies under this binning of these 16 fixed design points, which does not establish that they are equivalent. Halving the diluting signal across two criteria cut success from " + _f2(_CNC.A_dil1_enr1.rows[3].p_lower) + " to " + _f2(_CNC.D_dil2_enr1.rows[3].p_lower) + " at 18 000 fitting patients, but it also roughly halves the attainable improvement, so the two cannot be separated in that design. Omitting one confounder returned apparent reliability to randomised levels while inflating the estimator's population limit by " + (_CL ? _pc(_CL.inflation,1) : "\u2014") + " and cutting correct selection from " + (_CFD ? f3(_CFD.rec_rand_5167) : "\u2014") + " to " + (_CFD ? f3(_CFD.rec_unmeas_5167) : "\u2014") + ".")]));
