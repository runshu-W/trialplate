...
if (!is.null(fr)) {
  ...
  ## Reviewer round 7, major point 2. The currency comparison bins the scenarios by
  ## each candidate currency and averages the within-bin standard deviation of the
  ## success probability. The three summaries came out similar, and we described that
  ## as the currencies being indistinguishable. That is more than a descriptive
  ## statistic over sixteen scenarios can carry, and the supplement never showed the
  ## binning. Everything the comparison rests on is exported here: the bin edges, the
  ## membership of each scenario, the within-bin standard deviations and the exact
  ## aggregation, so a reader can see how few scenarios are in each bin.
  spread_detail <- function(v, nm) {
    q  <- stats::quantile(v, c(0,.25,.5,.75,1), na.rm = TRUE)
    ed <- unique(q)
    z  <- cut(v, ed, include.lowest = TRUE)
    sds <- tapply(rows$p_lower, z, stats::sd)
    ns  <- tapply(rows$p_lower, z, function(u) sum(is.finite(u)))
    list(currency = nm, edges = unname(ed),
         bins = lapply(seq_along(levels(z)), function(i) {
           rr <- unique(rows$run[as.integer(z) == i])
           list(bin = levels(z)[i],
                n_rows = unname(ns[i]),          # rows: scenario x fitting size
                n_scenarios = length(rr),        # distinct scenarios
                sd = unname(sds[i]), runs = rr) }),
         mean_within_sd = mean(sds, na.rm = TRUE),
         n_bins_used = sum(is.finite(sds)))
  }
  J$fixedn <- list(cells = fx,
    spread_patients = spread_detail(rows$n, "fitting patients")$mean_within_sd,
    spread_events   = spread_detail(rows$events, "full-protocol events")$mean_within_sd,
    spread_ess      = spread_detail(rows$ess, "effective sample size")$mean_within_sd,
...
## Reviewer round 7, major point 1. A percentile interval taken on the raw outer
## distribution is too wide, because each outer replicate is an R-split estimator and
## its spread is Var_outer + E[s^2/R], not Var_outer. We report both:
##
##   raw       percentiles of the outer replicate means, as before
##   deflated  percentiles after shrinking each replicate toward the mean by
##             sqrt( max(v_tot - v_mc, 0) / v_tot ), which removes the inner Monte
##             Carlo component from the SPREAD while leaving the centre and the shape
##             of the distribution alone
##
## and the Monte Carlo uncertainty of the endpoints themselves, from resampling the
## outer replicates. The deflation is an approximation: it assumes the inner noise is
...
if (!is.null(ctr)) J$criteria <- list(rows = ctr$rows, head = ctr$head,
  corr = if (!is.null(ctr$corr)) lapply(ctr$corr, function(M)
    list(names = colnames(M), m = lapply(seq_len(nrow(M)), function(i) unname(M[i, ])))) else NULL)
...
## Reviewer round 7, minor point 6. The comparator set excludes the rule's own
## subset and all comparisons are strict, so an exact tie counts against the rule.
## We asserted ties were negligible without measuring them; they are measured here.
...
## is stored as a named list. Accept either shape.
if (!is.null(scv) && !is.null(scv$rows)) scv <- scv$rows
if (!is.null(scv)) {
  J$converge <- list(rows = scv,
    counts = sort(unique(vapply(scv, function(z) as.numeric(z$R), numeric(1)))),
    n_used = max(vapply(scv, function(z) as.numeric(z$n_used), numeric(1))))
  ## The exact Monte Carlo SD at the bootstrap's own inner split count should equal
  ## the estimate the nested runs form internally from the within-resample variance.
  ## Agreement is what licenses the deflation, so it is computed and reported.
  Rb <- if (!is.null(J$nested$colon)) J$nested$colon$R else 25
  cmp <- lapply(scv[vapply(scv, function(z) z$R == Rb, logical(1))], function(z) {
    ref <- if (!is.null(J$nested$colon[[z$quantity]])) J$nested$colon[[z$quantity]]$sd_mc
           else if (!is.null(J$threeway_nested$colon[[z$quantity]])) J$threeway_nested$colon[[z$quantity]]$sd_mc
           else NULL
    if (is.null(ref)) return(NULL)
    list(quantity = z$quantity, exact = z$mc_sd, internal = ref, diff = abs(z$mc_sd - ref))
  })
  cmp <- cmp[!vapply(cmp, is.null, logical(1))]
  J$converge$vs_internal <- list(R = Rb, rows = cmp,
    max_diff = if (length(cmp)) max(vapply(cmp, function(z) z$diff, numeric(1))) else NA_real_)
}
