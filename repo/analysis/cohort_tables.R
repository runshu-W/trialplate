...
describe <- function(d, crit, ps, trt, tm, st, tau, key, label) {
  ...
  ## Reviewer round 7, minor 5: the supplement gave only the largest off-diagonal
  ## value. The matrix itself is small enough to print, so keep it.
...
sink()
saveRDS(list(rows = ROWS, head = HEAD, corr = CORR), "analysis/out/cohort_table_rows.rds")
