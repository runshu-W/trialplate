...
/* ---------------------------------------------------------------- 1 */
add(H("一、一句话"));
add(P("Trial Pathfinder（Liu 等，Nature 2021）用 Shapley 值找出「限制了人数却没带来疗效」的入组标准并放宽，宣称由此既能多纳入病人、又能得到更好的疗效估计。我们把这两个承诺分开做了样本外检验，四轮审稿之后的结论是：**多纳入病人这一条主要由规则的构造决定**——把规则放到「所有 512 个子集」里去比，在相同入组人数下它是否还有优势，**这两个队列的规模判不出来**：点估计在 colon 是 " + pct(C["hr0.1"]) + "、Rotterdam 是 " + pct(RT["hr0.1"]) + "，但把病人重抽的波动算进来之后，这个比例的中间 80% 范围约占单位区间的三分之一，即对队列构成相当敏感。它落在「人数—风险比」经验前沿上的比例只有 " + pct(C.front_hr) + " 和 " + pct(RT.front_hr) + "；不过这个前沿本身是从同一批留出数据里挑最优挑出来的，带乐观偏倚，我们另外量化了这个偏倚有多大。至于疗效估计要多少数据才够：在我们模拟的这些机制里，**单靠队列人数、事件数或有效样本量都定不出一个通用的门槛**，而且我们也**说不出决定它的是哪个量**（第六轮审稿指出 16 个情景只能说明机制间有异质性，不能证明这种要求「不存在」，标题已改）——第三轮我们曾说是「最强的那条稀释性标准」，这一条已经撤回。这是一个否定性结论，也是我们认为最有用的一条。", { after: 140 }));
...
add(P("公平的对照是把**全部 512 个子集**都在留出的那一半上评分（第一轮用「随机去掉同样条数」不公平，因为两者纳入的人数差很多）。在合格人数相差 10% 以内的子集中，规则的风险比更低的比例是 " + pct(C["hr0.1"]) + "（colon）和 " + pct(RT["hr0.1"]) + "（Rotterdam）" + (NS.colon ? "，重抽病人时中间 80% 的范围分别是 [" + f(NS.colon["hr0.1"].ci80[0],2) + "\u2013" + f(NS.colon["hr0.1"].ci80[1],2) + "] 和 [" + f(NS.rott["hr0.1"].ci80[0],2) + "\u2013" + f(NS.rott["hr0.1"].ci80[1],2) + "]（不是置信区间）" : "") + "；在 RMST 上是 " + pct(C["rm0.1"]) + " 和 " + pct(RT["rm0.1"]) + "。**重抽病人时这些范围各自约占单位区间的四分之一到三成**——我们不把它当检验，文中也没有做过任何有校准的总体层面比较。这是我们第三轮之后最重要的一处措辞改动。"));
...
add(TBL(["样本外对比","colon (n=619)","Rotterdam (n=2982)"],
 ...
 [2900,2300,2300],
 "最后一行来自嵌套自助（" + NS.colon.B + " 次外层重抽 × " + NS.colon.R + " 次内层划分），第八轮起不再称其为区间：报告的是外层自助分布的中间 80%「患者重抽范围」，覆盖率未经验证。第二轮审稿指出原实现有泄漏——自助样本里同一病人的副本可能落到划分两侧；改成按病人 ID 划分后区间反而变窄。总标准差 " + f(NS.colon.lower.sd_total,3) + " 中，内层蒙特卡洛成分 " + f(NS.colon.lower.sd_mc,3) + "、外层抽样成分 " + f(NS.colon.lower.sd_outer,3) + "。"));
...
/* ---------------------------------------------------------------- 5 */
add(H("五、四轮审稿指出的错误"));
add(B("错误一、二", "原稿说 Shapley 效率公理在风险比尺度上只近似成立，**不成立**：穷举枚举下任何尺度都精确成立（" + E.err_log.toExponential(1) + " 和 " + E.err_hr.toExponential(1) + "）。同一稿又把「多纳入病人」当成经过验证的收益，其实是构造性的；两处均已改写。"));
add(B("漏查", "Rotterdam 在完整方案内存在**正性假设问题**：419 人里 132 个倾向值顶在截断边界，加权有效样本量只有名义的 37%；化疗几乎只给了年轻绝经前女性。该队列现在全部按描述性报告，因果解读只靠随机化队列和模拟。"));

add(B("错误三、四", "第二轮：嵌套自助有泄漏——同一病人的副本可能落到划分两侧；已按病人 ID 划分并加断言。同一轮里，只用「能观察到阈值」的 6 个情景算离散度有选择偏倚，结论**不成立**，已撤回。"));
...
add(B("错误七（最严重）", "第四轮：正文和补充材料对同一个量给出不同的数，我们上一轮解释成蒙特卡洛噪声。真正原因要糟得多——第二、三轮新增的 **11 个脚本把 Rotterdam 的终点写成「无复发生存时间 + 死亡指示」并在 1825 天截断**，而表 1 用的是「死亡时间 + 死亡指示」在 2555 天截断。时间和事件指示不配对会系统性缩短事件时间。这是两个不同的终点，不是两次运行的差异。已全部改正并重算。"));
add(B("错误八", "第七轮，也是最锋利的一条统计意见：三分法那几个量的**区间和点估计并不是同一个统计量**——外层自助每抽一次只做 25 次内层划分，点估计却用 400/500 次，区间因此混进了内层蒙特卡洛方差、偏宽。我们注意到了症状（两次运行最多差 0.14）却没顺着它得出结论。新增表 S1e 把长运行切成互不重叠的小块，给出**独立 R 次划分估计的实测离散度和均值**，而不只是 s/√R 这个恒等式；表 S3、S3c 同时给出原始与压缩后的区间及端点误差。confirmatory（确证）一词已全部撤下。"));
add(B("错误九", "第七轮自查：三种「信息货币」的差值报的是**自助均值**而非观察值；每次重抽都重切四分位边界，两者不等且符号相反。已改报观察值。"));

add(B("错误十（这一轮最重要）", "第八轮：压缩后的区间**不是验证过覆盖率的 95% 患者层面置信区间**。线性收缩只匹配到二阶矩，不能从「患者变异 ⊗ 蒙特卡洛噪声」的卷积里还原患者抽样分位数；统计量有界，流程里还有 Shapley 取符号和前沿挑选两个非光滑步骤。我们一边声明覆盖率未验证、一边用「是否跨过 0.5」下结论，仍是把未校准区间当正式推断。现全文不再出现「95% 区间」「患者层面区间」「置信区间」，改称**患者重抽范围**，并**删去所有依据是否跨过 0.5 或 0 的判断**；又因 100–150 次外层重抽下 2.5%／97.5% 端点只由第 3 个观测决定，改以**中间 80% 范围**为主。"));
add(B("错误十一", "第八轮：方差差为非正时，正确结论是**当前蒙特卡洛精度下患者层面方差分量不可辨识**，既不能说它是零，也不能说「内层项吃掉了全部离散度」。前后两次都读错了，已在正文和表 S3、S3c 改正并注明撤回。"));
add(B("错误十二", "第八轮：16 个情景是部分因子设计的**固定设计点**，不是从机制总体抽出的 cluster，对它们做整群自助没有可指涉的抽样总体。整群自助已**撤回**，改为固定设计下的**留一情景敏感性分析**（表 S2c），结论收窄为「在这一分箱下三个描述性汇总数值相近」。"));
...
add(B("蕴含检测器", "若标准 i 逻辑上蕴含 j，这对的交互是算术恒等式，无论样本量多大都无信息，必须剔除。"));
add(B("双估计量并行", "同时报告风险比和 RMST 差。条件风险比固定为 0.6 的模拟里，一条没改变任何人相对获益的标准，其对数风险比 Shapley 值却随预后异质性从 −0.0008 长到 −0.066。"));
...
add(P("因为入组人数通常不是唯一目的。规则换来的可能是估计质量，代价是少招一些人。但在**相同入组人数**下是否仍有优势，这两个队列的规模判不出来；只跟完整方案比是看不出这个取舍的。"));
add(SUB("实际操作上我该怎么做？"));
add(P("四步。（1）先把安全性、耐受性、给药可行性相关的标准标记为**不可放宽**。（2）看结局之前先算交互杠杆和逻辑蕴含。（3）检查完整方案内部的重叠。（4）看有没有一条标准集中了足够差异获益；没有就只报告人群扩大，不报告疗效改善。"));

const doc = new Document({
  sections: [{
    properties: { page: {
      size: { width: 12240, height: 15840 },
      margin: { top: 600, bottom: 540, left: 860, right: 860 }
    }},
    children: kids
  }]
});
