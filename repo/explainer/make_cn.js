...
const P = (t, o = {}) => new Paragraph({
  spacing: { after: o.after ?? 68, line: 254 },
...
const B = (mark, t) => new Paragraph({
  spacing: { after: 52, line: 258 }, alignment: AlignmentType.JUSTIFIED,
...
/* ---------------------------------------------------------------- 1 */
add(H("一、一句话"));
add(P("Trial Pathfinder（Liu 等，Nature 2021）用 Shapley 值找出「限制了人数却没带来疗效」的入组标准并放宽，宣称由此既能多纳入病人、又能得到更好的疗效估计。我们把这两个承诺分开做了样本外检验，四轮审稿之后的结论是：**多纳入病人这一条主要由规则的构造决定**——把规则放到「所有 512 个子集」里去比，在相同入组人数下它是否还有优势，**这两个队列的规模判不出来**：点估计在 colon 是 " + pct(C["hr0.1"]) + "、Rotterdam 是 " + pct(RT["hr0.1"]) + "，但把不确定性做到病人层面之后，区间都包含 0.5。它落在「人数—风险比」经验前沿上的比例只有 " + pct(C.front_hr) + " 和 " + pct(RT.front_hr) + "；不过这个前沿本身是从同一批留出数据里挑最优挑出来的，带乐观偏倚，我们另外量化了这个偏倚有多大。至于疗效估计要多少数据才够：在我们模拟的这些机制里，**单靠队列人数、事件数或有效样本量都定不出一个通用的门槛**，而且我们也**说不出决定它的是哪个量**（第六轮审稿指出 16 个情景只能说明机制间有异质性，不能证明这种要求「不存在」，标题已改）——第三轮我们曾说是「最强的那条稀释性标准」，这一条已经撤回。这是一个否定性结论，也是我们认为最有用的一条。", { after: 140 }));
...
add(P("但审稿人第三轮指出：这个前沿是**在同一批留出数据上从 512 个带噪声的估计里挑最优挑出来的**，带「胜者诅咒」式的乐观偏倚。真值已知的模拟给出量级：表面支配者里只有 " + (FO?pct(FO.true_frac):"—") + " 在总体上真的支配，" + (FO?pct(FO.repro):"—") + " 能在另一半独立数据上重现。" + (TW ? "第四轮我们又在真实队列里做了**三分法**。第五轮审稿要求把三个量分开：(a) 挑选那一份上的支配者，是乐观的经验量；(b) **把这些预先选定的子集拿到没用过的那一份上重新评价，这是唯一的确证量**——colon 只有 " + pct(TW.colon.repro) + " 仍然支配，规则未被任何预选子集支配的比例是 " + pct(TW.colon.front_conf) + "（Rotterdam " + pct(TW.rott.front_conf) + "）" + (TN ? "，病人层面区间 [" + f(TN.colon.front_conf.ci[0],2) + ", " + f(TN.colon.front_conf.ci[1],2) + "] 和 [" + f(TN.rott.front_conf.ci[0],2) + ", " + f(TN.rott.front_conf.ci[1],2) + "]" : "") + "，而同一个三分设计下挑选份上的经验口径只有 " + pct(TW.colon.front_sel) + "／" + pct(TW.rott.front_sel) + "（第六轮审稿要求和同一设计比，不能跟对半划分比，后者同时改变了评价集大小）；(c) 在检验那一份上重新扫描全部 512 个子集，仍然是经验最优，只作描述。方向不变（大多数划分里规则仍被支配），但只报 (a) 会把差距夸大。" : "") + "所以现在的说法是：规则离**这个带乐观偏倚的经验最优**有相当距离；它离真正的最优方案有多远，这两个队列答不了。"));
...
/* ---------------------------------------------------------------- 5 */
add(H("五、四轮审稿指出的错误"));
add(B("错误一、二", "原稿说 Shapley 效率公理在风险比尺度上只近似成立，**不成立**：穷举枚举下任何尺度都精确成立（验证到 " + E.err_log.toExponential(1) + " 和 " + E.err_hr.toExponential(1) + "）。同一稿把「多纳入病人」当成经过验证的收益，其实是构造性的（见发现 1）；两处均已全文改写。"));
add(B("漏查", "Rotterdam 在完整方案内存在**正性假设问题**：419 人里 132 个倾向值顶在截断边界，加权有效样本量只有名义的 37%；化疗几乎只给了年轻绝经前女性。该队列现在全部按描述性报告，因果解读只靠随机化队列和模拟。"));

add(B("错误三、四", "第二轮：嵌套自助有泄漏——自助样本里同一病人的副本可能落到划分两侧；已按病人 ID 划分并加断言，重算后区间变窄。同一轮里，只用「能观察到阈值」的 6 个情景算离散度有选择偏倚，据此得出的结论**不成立**，已撤回。"));
add(B("错误五", "第二轮：非可折叠性一节写「被选中的低风险人群有更多绝对生存空间可赚」，**方向反了**——RMST 差 0.664→0.599、0.425→0.322，低风险人群绝对获益更少。"));

add(B("错误六", "第三轮：我们写「把富集摊薄没关系」，其实在 6000 人处是有帮助的（+0.21，区间 [0.08, 0.34]）；第七轮自查发现改后的「摊薄从不吃亏」同样过头——只有 6000 人这一档区间不含零，600 人那档点估计还是反向的，现已按档说明。同节「稀释系数是决定性的量」也撤回：系数和可达改进幅度在设计上绑定。"));
add(B("错误七（最严重）", "第四轮：正文和补充材料对同一个量给出不同的数，我们上一轮把它解释成蒙特卡洛噪声。真正的原因要糟得多——第二、三轮新增的 **11 个脚本把 Rotterdam 的终点写成了「无复发生存时间 + 死亡指示」在 1825 天截断**，而表 1 用的是正确的「死亡时间 + 死亡指示」在 2555 天截断。时间和事件指示不配对，会系统性缩短事件时间。这不是两次运行的差异，是两个不同的终点。已全部改正并重算。"));
add(B("错误八", "第七轮，也是最锋利的一条统计意见：三分法那几个量的**区间和点估计并不是同一个统计量**——外层自助每抽一次只做 25 次内层划分，点估计却用 400/500 次，区间因此混进了内层蒙特卡洛方差、偏宽。我们注意到了症状（两次运行最多差 0.14）却没顺着它得出结论。新增的表 S1e 把长运行切成互不重叠的小块，给出**独立的 R 次划分估计的实测离散度和均值**，而不是只给 s/√R 这个恒等式；表 S3、S3c 对每个量同时给出原始区间和压掉内层方差后的区间，并报告端点自身的蒙特卡洛误差。口径也改了：这些区间**近似、覆盖率未经验证**，confirmatory（确证）一词已全部撤下。"));
add(B("错误九", "第七轮自查：三种「信息货币」（人数／事件数／有效样本量）差值报的是**自助均值**而非观察值。每次重抽都会重切四分位边界，两者不但不等、符号还相反。现改报观察值，自助只给区间。"));
...
add(B("交互杠杆", "闭式量 L = 1 − p(ij)/p(i) − p(ij)/p(j) + p(ij)，只用入组率、不用结局数据。原稿把「杠杆低」说成「结构上测不出」**不够准确**：它是按已知因子的衰减，信号仍取决于效应大小和样本量；真正测不出的只有逻辑蕴含。"));
add(B("蕴含检测器", "若标准 i 逻辑上蕴含 j，这对的交互是算术恒等式，无论样本量多大都无信息，必须从检验族里剔除。"));
add(B("双估计量并行", "同时报告风险比和 RMST 差。条件风险比固定为 0.6 的模拟里，一条没改变任何人相对获益的标准，其对数风险比 Shapley 值却随预后异质性从 −0.0008 长到 −0.066；异质性为零时假象消失。"));
...
add(P("因为入组人数通常不是唯一目的。规则换来的可能是估计质量，代价是比它能做到的少招一些人。但在**相同入组人数**下它是否仍有这个优势，这两个队列的规模判不出来（区间都包含 0.5）。这个取舍只跟完整方案比是显现不出来的。"));
add(SUB("实际操作上我该怎么做？"));
add(P("四步。（1）先把出于安全性、耐受性、给药可行性的标准标记为**不可放宽**。（2）看结局之前先算交互杠杆和逻辑蕴含。（3）检查完整方案内部的正性/重叠。（4）判断有没有一条标准集中了足够差异获益；没有的话，只报告人群扩大，不报告疗效改善。"));

const doc = new Document({
  sections: [{
    properties: { page: {
      size: { width: 12240, height: 15840 },
      margin: { top: 700, bottom: 660, left: 950, right: 950 }
    }},
    children: kids
  }]
});
