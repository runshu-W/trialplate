...
/* ---------------------------------------------------------------- 1 */
add(H("一、一句话"));
add(P("Trial Pathfinder（Liu 等，Nature 2021）用 Shapley 值找出「限制了人数却没带来疗效」的入组标准并放宽，宣称由此既能多纳入病人、又能得到更好的疗效估计。我们把这两个承诺分开做了样本外检验，四轮审稿之后的结论是：**多纳入病人这一条主要由规则的构造决定**——把规则放到「所有 512 个子集」里去比，在相同入组人数下它是否还有优势，**这两个队列的规模判不出来**：点估计在 colon 是 " + pct(C["hr0.1"]) + "、Rotterdam 是 " + pct(RT["hr0.1"]) + "，但把病人重抽的波动算进来之后，这个比例的中间 80% 范围约占单位区间的三分之一，且横跨 0.5。它落在「人数—风险比」经验前沿上的比例只有 " + pct(C.front_hr) + " 和 " + pct(RT.front_hr) + "；不过这个前沿本身是从同一批留出数据里挑最优挑出来的，带乐观偏倚，我们另外量化了这个偏倚有多大。至于疗效估计要多少数据才够：在我们模拟的这些机制里，**单靠队列人数、事件数或有效样本量都定不出一个通用的门槛**，而且我们也**说不出决定它的是哪个量**（第六轮审稿指出 16 个情景只能说明机制间有异质性，不能证明这种要求「不存在」，标题已改）——第三轮我们曾说是「最强的那条稀释性标准」，这一条已经撤回。这是一个否定性结论，也是我们认为最有用的一条。", { after: 140 }));
...
add(P("公平的对照是把**全部 512 个子集**都在留出的那一半上评分（第一轮用「随机去掉同样条数」不公平，因为两者纳入的人数差很多）。在合格人数相差 10% 以内的子集中，规则的风险比更低的比例是 " + pct(C["hr0.1"]) + "（colon）和 " + pct(RT["hr0.1"]) + "（Rotterdam）" + (NS.colon ? "，病人层面的区间分别是 [" + f(Math.max(0,C["hr0.1"]-1.96*NS.colon["hr0.1"].sd_outer),2) + ", " + f(Math.min(1,C["hr0.1"]+1.96*NS.colon["hr0.1"].sd_outer),2) + "] 和 [" + f(Math.max(0,RT["hr0.1"]-1.96*NS.rott["hr0.1"].sd_outer),2) + ", " + f(Math.min(1,RT["hr0.1"]+1.96*NS.rott["hr0.1"].sd_outer),2) + "]" : "") + "；在 RMST 上是 " + pct(C["rm0.1"]) + " 和 " + pct(RT["rm0.1"]) + "。**重抽病人时这些范围各自约占单位区间的四分之一到三成，且横跨 0.5**——我们不把它当检验，也不据此对总体值下任何结论。这是我们第三轮之后最重要的一处措辞改动。"));
